...
def sortArgs(listOfArgs):
    ...
    for i, arg in enumerate(listOfArgs):
        ...
        elif arg == "--restrict-class":
            restrictClass = listOfArgs[i+1]
    ...
    return (replaceInlineMath, replaceClasses, extractClasses, noPunc,
            puncFile, outputFile, inputFile, restrictClass)

def compileRegex(replaceInlineMath, replaceClasses, noPunc, puncFile, extractClasses, restrictClass):
    ...
    restrictRE = False
    if restrictClass:
        restrictRE = (re.compile(r'<span class=\'envir\'>\\begin<\/span><span class=\'envir\'>\{'+restrictClass+r'\}<\/span>.*?<span class=\'envir\'>\\end<\/span><span class=\'envir\'>\{'+restrictClass+r'\}<\/span>', re.DOTALL))

    deleteRemaining = re.compile(r'<span class=[^>]+>([^<])+</span>')
    return replace, extract, puncRe, deleteRemaining, restrictRE

def extractText(inFile, repRE, extRE, puncRE, delRE, restrictRE):
    try:
        os.chdir('..')
        ...
        if restrictRE:
            text = ''.join(restrictRE.findall(text))
...
def main():
    ...
    if "--help" in arguments:
        ...
        if sArgs[6]:
            ...
            repRE, extRE, puncRE, delRE, restrictRE = compileRegex(sArgs[0], sArgs[1], sArgs[3],
                                               sArgs[4], sArgs[2], sArgs[7])
            toSave = extractText(sArgs[6], repRE, extRE, puncRE, delRE, restrictRE) 
                ...
                for filename in files:
                    ...
                    if filename.endswith(".tex"):
                        ...
                        outFile = inFile.replace(rootPath, newRoot).replace(
                            '.tex','.txt')
                        checkDirectoryExists(outFile)
                        toSave = extractText(inFile, repRE, extRE, puncRE, delRE, restrictRE)
                        if toSave != -1:
                            fileToSave = open(outFile, "w+")
                            try:
                                fileToSave.write(toSave)
...
--extract-class "class"
  ...
  are word and hword
-- restrict-class "class" 
  only include things to be extracted from within one class environment
