...
def extractIds(identifiers, subIds, xmlDoc):
    ...
    for item in recordList:
        try:
            ...
            e = sys.exc_info()[0]
            try:
                print getText(item.getElementsByTagName('identifier')[0].childNodes)
            except:
                pass
            print str(e)
...
---------------------------------------------------
Usage:
createDictIDs.py [options] [input folder]
Options:
 --debug     print filename being processed, useful
             for catching malformed data
---------------------------------------------------
"""
